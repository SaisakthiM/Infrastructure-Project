...
// resolveDockerHost returns the configured docker host, or a detected/
// hardcoded fallback if it was never set.
func resolveDockerHost(cfg *config.Config) string {
	if strings.TrimSpace(cfg.DockerHost) != "" {
		return cfg.DockerHost
	}
	return dockerhost.Default()
}

// GenerateAll writes terraform.tfvars for the environments that need them,
// plus the docker_host value into the static promtail config.
func GenerateAll(cfg *config.Config) error {
	...
	if err := updatePromtailDockerHost(cfg); err != nil {
		return err
	}
	return nil
}
...
docker_host = %q
`, resolveDockerHost(cfg))
	return write(filepath.Join(envPath, "prod-gateway", "terraform.tfvars"), content)
}

// updatePromtailDockerHost rewrites the docker_sd_configs host line in the
// static promtail config used by the docker-compose-style stack, so it
// matches the configured docker host instead of staying hardcoded.
func updatePromtailDockerHost(cfg *config.Config) error {
	path := filepath.Join(cfg.InfraDir, "projects", "Social Media App", "platform", "observability", "promtail-config.yml")
	...
	updated := dockerHostLineRe.ReplaceAllString(string(raw), "${1}"+resolveDockerHost(cfg))
...
var dockerHostLineRe = regexp.MustCompile(`(?m)^(\s*-\s*host:\s*)\S+`)
